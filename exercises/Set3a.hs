...
powers :: Int -> Int -> [Int]
powers k max = map (k^) (takeWhile (\i -> (k ^ i) <= max) [0..])
...
whileRight f x = check $ f x
    where
        check (Right a) = whileRight f a
        check (Left b) = b
...
joinToLength n xs = filter ((== n) . length) [x ++ x' | x <- xs, x' <- xs]
...
sumRights xs = sum $ map (either (return 0) id) xs
...
interpreter [] = []
interpreter commands = filter (not . null) $ runCommands commands (0, 0)

runCommands :: [String] -> (Int, Int) -> [String]
runCommands [] (_, _) = []
runCommands commands (x, y) =  ppos : runCommands (drop 1 commands) pos
    where
        ppos = printPos command (x, y)
        pos = move command (x, y)
...
printPos :: String -> (Int, Int) -> String
printPos "printX" (x, _) = show x
printPos "printY" (_, y) = show y
printPos _ (_, _) = ""
