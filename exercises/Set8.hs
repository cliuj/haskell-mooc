...
blendColor (Color r g b) (Color r' g' b') = Color (avg r r') (avg g g') (avg b b')
    where
        avg x x' = (x + x') `div` 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine b (Picture f') (Picture f'') = Picture f
    where
        f coord = b (f' coord) (f'' coord)
...
union (Shape s) (Shape s') = Shape f
    where
        f coord = s coord || s' coord

cut :: Shape -> Shape -> Shape
cut (Shape s) (Shape s') = Shape f
    where
        f coord = not $ s coord && s' coord
------------------------------------------------------------------------------
...
instance Transform Zoom where
  apply (Zoom int) picture = zoom int picture

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
    apply FlipX (Picture picture) = Picture f
        where
            f (Coord x y) = picture $ Coord (-x) y
    apply FlipY (Picture picture) = Picture f
        where
            f (Coord x y) = picture $ Coord x (-y)
...
blurColor colors = avg (foldr add (Color 0 0 0) colors) (length colors) 
    where
        add (Color r g b) (Color r' g' b') = Color (r+r') (g+g') (b+b')
        avg (Color r g b) n = Color (r `div` n) (g `div` n) (b `div` n)

instance Transform Blur where
    apply Blur (Picture picture) = Picture f
        where
            f (Coord x y) = blurColor $ map picture neighbors
                where
                    neighbors = [ Coord x y
                                , Coord (x + 1) y
                                , Coord (x - 1) y
